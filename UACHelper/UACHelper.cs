...
namespace UACHelper
{
    ...
    public static class UACHelper
    {
        ...
        /// <summary>
        ///     A <see cref="Boolean" /> value indicating if the user that owns this process also owns the desktop session
        /// </summary>
        public static bool IsDesktopOwner
            =>
                CachedOwner.User?.Equals(
                    (SecurityIdentifier) DesktopOwner.Translate(typeof (SecurityIdentifier))) == true;
        ...
        public static bool IsVirtualized
            => Tokens.GetTokenElevationType(CachedOwner.Token) != TokenElevationType.Default;

        /// <summary>
        ///     A <see cref="Boolean" /> value indicating if the user that owns this process is a member of the 'Administrators'
        ///     group
        /// </summary>
        ...
        /// <summary>
        ///     A <see cref="Boolean" /> value indicating if the UAC vitalization is enable on this machine
        /// </summary>
        ...
        /// </summary>
        public static NTAccount DesktopOwner => GetProcessDesktopOwner(Process.GetCurrentProcess());
        ...
        public static NTAccount GetProcessDesktopOwner(Process process)
        {
            try
            {
                ...
                int bufferSize;
                if (
                    WindowsTerminal.QuerySessionInformation(IntPtr.Zero, sessionId, WindowsTerminalInfoClass.UserName,
                        out buffer, out bufferSize) && bufferSize > 0)
                {
                    ...
                    {
                        if (
                            WindowsTerminal.QuerySessionInformation(IntPtr.Zero, sessionId,
                                WindowsTerminalInfoClass.DomainName, out buffer, out bufferSize) && bufferSize > 0)
                        {
        ...
        public static Process StartWithShell(ProcessStartInfo startInfo)
        {
            ...
            uint shellProcessId;
            if (User.GetWindowThreadProcessId(shellWindow, out shellProcessId) == 0 || shellProcessId == 0)
            {
                throw new Win32Exception();
            }
        ...
        {
            RunLevelConclusionReason reason;
            return GetExpectedRunlevel(applicationAddress, out reason);
        }
    }
}